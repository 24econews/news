"""Post all countries' daily digests to Bluesky, staggered 30 minutes apart."""
...
BLUESKY_POSTER = os.path.join(SCRIPT_DIR, "bluesky_poster.py")
...
def post_country(country: str, date_str: str, dry_run: bool) -> bool:
    ...
    cmd = [sys.executable, BLUESKY_POSTER, "--country", country, "--date", date_str]
    if dry_run:
        cmd.append("--dry-run")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Post all countries' digests to Bluesky, staggered 30 minutes apart.")
    parser.add_argument("--date", default=date.today().isoformat(), help="YYYY-MM-DD, defaults to today")
    parser.add_argument("--dry-run", action="store_true", help="Compose and print posts without posting or waiting")
    args = parser.parse_args()

    logger.info(f"=== post_all_countries_bluesky started for {args.date} ===")
    ...
    logger.info(f"=== post_all_countries_bluesky finished: {len(succeeded)} succeeded, {len(failed)} failed ===")
    if succeeded:
